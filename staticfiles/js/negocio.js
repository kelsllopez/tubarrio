let state = {
  ...
  tipoHorario: 'unico', // 'unico' o 'diferenciado'
  diasUnico: [], desdeUnico: '09:00', hastaUnico: '18:00',
  horariosDiferenciados: [], // Array de objetos {id, dias: [], desde: '', hasta: ''}
  wsp:'',
  lat: null, lng: null,
  descripcion: '',
  photos: [],
...
let horarioCounter = 0; // Contador para IDs únicos
...
function cambiarTipoHorario(tipo) {
  console.log('Cambiando tipo de horario a:', tipo);
  state.tipoHorario = tipo;
  
  const tipoSeleccionado = document.getElementById('tipoHorarioSeleccionado');
  if (tipoSeleccionado) tipoSeleccionado.value = tipo;
  
  const seccionUnico = document.getElementById('horarioUnicoSection');
  const seccionDiferenciado = document.getElementById('horarioDiferenciadoSection');
  
  if (tipo === 'unico') {
    seccionUnico.style.display = 'block';
    seccionDiferenciado.style.display = 'none';
  } else {
    seccionUnico.style.display = 'none';
    seccionDiferenciado.style.display = 'block';
    
    // Si no hay horarios diferenciados, crear el primero por defecto
    if (state.horariosDiferenciados.length === 0) {
      agregarNuevoHorario();
    } else {
      renderizarHorariosDiferenciados();
    }
  }
...
function agregarNuevoHorario() {
  ...
  state.horariosDiferenciados.push(nuevoHorario);
  renderizarHorariosDiferenciados();
  updatePreview();
}

function eliminarHorario(id) {
  const index = state.horariosDiferenciados.findIndex(h => h.id === id);
  if (index > -1) {
    state.horariosDiferenciados.splice(index, 1);
    renderizarHorariosDiferenciados();
    updatePreview();
  }
}

function renderizarHorariosDiferenciados() {
  const container = document.getElementById('horariosContainer');
  if (!container) {
    console.error('No se encontró el contenedor horariosContainer');
    return;
  }
  ...
  state.horariosDiferenciados.forEach((horario, index) => {
    const horarioDiv = document.createElement('div');
    horarioDiv.className = 'horario-section';
    horarioDiv.setAttribute('data-horario-id', horario.id);
    
    const numeroHorario = index + 1;
    
    horarioDiv.innerHTML = `
      <div class="section-title">📌 Horario ${numeroHorario}</div>
      
      ${index > 0 ? `
        <button type="button" class="btn-remove-horario" data-horario-id="${horario.id}" title="Eliminar este horario">
          ✕ Eliminar
        </button>
      ` : ''}
      
      <div class="field">
        <label>¿Qué días abrís? <span class="req">*</span></label>
        <div class="days-grid" id="daysGrid-${horario.id}">
          <button type="button" class="day-btn" data-day="Lun" data-horario-id="${horario.id}">Lun</button>
          <button type="button" class="day-btn" data-day="Mar" data-horario-id="${horario.id}">Mar</button>
          <button type="button" class="day-btn" data-day="Mié" data-horario-id="${horario.id}">Mié</button>
          <button type="button" class="day-btn" data-day="Jue" data-horario-id="${horario.id}">Jue</button>
          <button type="button" class="day-btn" data-day="Vie" data-horario-id="${horario.id}">Vie</button>
          <button type="button" class="day-btn" data-day="Sáb" data-horario-id="${horario.id}">Sáb</button>
          <button type="button" class="day-btn" data-day="Dom" data-horario-id="${horario.id}">Dom</button>
        </div>
        <div class="selected-days-hint" id="daysDisplay-${horario.id}">
          ${horario.dias.length ? horario.dias.join(', ') : 'Seleccioná los días'}
        </div>
      </div>
      ...
      <div class="field">
        ...
        <div class="horario-wrap">
          <input type="time" id="desde-${horario.id}" value="${horario.desde}"/>
          <span class="horario-sep">hasta</span>
          <input type="time" id="hasta-${horario.id}" value="${horario.hasta}"/>
        </div>
      </div>
    `;
    ...
    horario.dias.forEach(day => {
      const btn = horarioDiv.querySelector(`[data-day="${day}"]`);
      if (btn) btn.classList.add('on');
    });
    
    // Agregar event listeners a los botones de días
    horarioDiv.querySelectorAll('.day-btn').forEach(btn => {
      btn.addEventListener('click', function() {
        const day = this.dataset.day;
        const horarioId = parseInt(this.dataset.horarioId);
        toggleDayDiferenciado(this, day, horarioId);
      });
    });
    
    // Agregar event listeners a los inputs de hora
    const desdeInput = horarioDiv.querySelector(`#desde-${horario.id}`);
    const hastaInput = horarioDiv.querySelector(`#hasta-${horario.id}`);
    
    if (desdeInput) {
      desdeInput.addEventListener('input', (e) => {
        horario.desde = e.target.value;
        updatePreview();
      });
    }
    ...
    if (hastaInput) {
      hastaInput.addEventListener('input', (e) => {
        horario.hasta = e.target.value;
        updatePreview();
      });
    }
    
    // Event listener para botón eliminar
    const removeBtn = horarioDiv.querySelector('.btn-remove-horario');
    if (removeBtn) {
  ...
  // Mostrar/ocultar botón de agregar según cantidad de horarios
  const btnAdd = document.getElementById('btnAddHorario');
  if (btnAdd) {
    btnAdd.style.display = state.horariosDiferenciados.length >= 5 ? 'none' : 'flex';
  }
}

function toggleDayDiferenciado(btn, day, horarioId) {
  btn.classList.toggle('on');
  
  const horario = state.horariosDiferenciados.find(h => h.id === horarioId);
  if (!horario) return;
  
  const idx = horario.dias.indexOf(day);
  if (idx > -1) {
    horario.dias.splice(idx, 1);
  } else {
    horario.dias.push(day);
  }
  
  const order = ['Lun','Mar','Mié','Jue','Vie','Sáb','Dom'];
  horario.dias.sort((a, b) => order.indexOf(a) - order.indexOf(b));
  
  // Actualizar display
  const display = document.getElementById(`daysDisplay-${horarioId}`);
  if (display) {
    display.textContent = horario.dias.length ? horario.dias.join(', ') : 'Seleccioná los días';
  }
  
  updatePreview();
}

function toggleDayUnico(btn, day) {
  btn.classList.toggle('on');
  
  const idx = state.diasUnico.indexOf(day);
  if (idx > -1) {
    state.diasUnico.splice(idx, 1);
  } else {
    state.diasUnico.push(day);
  }
  
  const order = ['Lun','Mar','Mié','Jue','Vie','Sáb','Dom'];
  state.diasUnico.sort((a, b) => order.indexOf(a) - order.indexOf(b));
  
  // Actualizar display
  const display = document.getElementById('daysDisplayUnico');
  if (display) {
    display.textContent = state.diasUnico.length ? state.diasUnico.join(', ') : 'Seleccioná los días';
  }
  
  updatePreview();
}

function getTodosLosDias() {
  if (state.tipoHorario === 'unico') {
    return state.diasUnico;
  } else {
    const todosDias = new Set();
    state.horariosDiferenciados.forEach(h => {
      h.dias.forEach(d => todosDias.add(d));
    });
    return Array.from(todosDias).sort((a, b) => {
      const order = ['Lun','Mar','Mié','Jue','Vie','Sáb','Dom'];
...
function getHorarioTexto() {
  if (state.tipoHorario === 'unico') {
    if (state.diasUnico.length > 0) {
      return `${state.diasUnico.join(', ')} · ${state.desdeUnico}–${state.hastaUnico}`;
    }
    ...
    state.horariosDiferenciados.forEach(h => {
      if (h.dias.length > 0) {
        textos.push(`${h.dias.join(', ')} · ${h.desde}–${h.hasta}`);
      }
    });
    return textos.length > 0 ? textos.join(' | ') : 'Horario no especificado';
  }
}

function getDiasAtencionCompleto() {
  if (state.tipoHorario === 'unico') {
    if (state.diasUnico.length > 0) {
      return `${state.diasUnico.join(', ')} · ${state.desdeUnico}–${state.hastaUnico}`;
    }
    ...
    state.horariosDiferenciados.forEach(h => {
      if (h.dias.length > 0) {
        textos.push(`${h.dias.join(', ')} de ${h.desde} a ${h.hasta}`);
      }
    });
    return textos.length > 0 ? textos.join(' | ') : 'No especificado';
  }
}
...
function validateStep(step) {
  if (step === 1) {
    ...
    // Validar días según tipo de horario
    let diasOk = false;
    if (state.tipoHorario === 'unico') {
      diasOk = state.diasUnico.length > 0;
      const errMsg = document.getElementById('horarioUnicoErr');
      if (errMsg) errMsg.style.display = diasOk ? 'none' : 'block';
    } else {
      diasOk = state.horariosDiferenciados.some(h => h.dias.length > 0);
      const errMsg = document.getElementById('horarioDiferenciadoErr');
      if (errMsg) errMsg.style.display = diasOk ? 'none' : 'block';
    }
...
function updatePreview() {
  ...
  const todosDias = getTodosLosDias();
  ...
  if (pceDias) pceDias.textContent = todosDias.length ? todosDias.join(', ') : 'No especificado';
  if (pceHora) pceHora.textContent = horarioTexto;
...
async function submitForm() {
  ...
  const diasAtencion = getDiasAtencionCompleto();
  ...
  formData.append('tipo_horario', state.tipoHorario);
  
  // Enviar horarios como JSON
  if (state.tipoHorario === 'unico') {
    formData.append('horario_1_dias', state.diasUnico.join(', '));
    formData.append('horario_1_inicio', state.desdeUnico);
    formData.append('horario_1_fin', state.hastaUnico);
  } else {
    formData.append('horarios_json', JSON.stringify(state.horariosDiferenciados));
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const tipoHorarioSelect = document.getElementById('tipoHorario');
  if (tipoHorarioSelect) {
    tipoHorarioSelect.addEventListener('change', (e) => {
      cambiarTipoHorario(e.target.value);
    });
  }
  ...
  if (btnAddHorario) {
    btnAddHorario.addEventListener('click', agregarNuevoHorario);
  }
  ...
  // Días de horario único
  document.querySelectorAll('#daysGridUnico .day-btn').forEach(el => {
    el.addEventListener('click', function() {
      const day = this.dataset.day;
      toggleDayUnico(this, day);
    });
  });

  // Listeners para inputs de horario único
  const desdeUnico = document.getElementById('inp-desde-unico');
  const hastaUnico = document.getElementById('inp-hasta-unico');
  if (desdeUnico) {
    desdeUnico.addEventListener('input', () => {
      state.desdeUnico = desdeUnico.value;
      updatePreview();
    });
  }
  if (hastaUnico) {
    hastaUnico.addEventListener('input', () => {
      state.hastaUnico = hastaUnico.value;
      updatePreview();
    });
  }
  ...
  // Inicializar con horario único
  cambiarTipoHorario('unico');
