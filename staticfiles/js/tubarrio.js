...
function verificarAbiertoAhora(diasStr) {
  ...
  try {
    ...
    // Dividir por el separador | (múltiples franjas horarias)
    const franjas = diasStr.split('|').map(f => f.trim());
    
    // Variables para seguimiento
    let abreHoy = false;
    let horarioAperturaHoy = null;
    let todosLosDiasNumeros = new Set();
    let horariosPorDia = {};
    
    // Para cada franja, verificar si hoy está incluido
    for (const franja of franjas) {
      // Intentar parsear formato nuevo: "días de HH:MM a HH:MM"
      let match = franja.match(/^(.+?)\s+de\s+(\d{1,2}):(\d{2})\s+a\s+(\d{1,2}):(\d{2})$/i);
      
      if (match) {
        const diasParte = match[1].trim();
        const horaInicio = parseInt(match[2]) * 60 + parseInt(match[3]);
        const horaFin = parseInt(match[4]) * 60 + parseInt(match[5]);
        
        const diasArray = diasParte.split(',').map(d => d.trim());
        const diasNumeros = diasArray.map(d => DIAS_MAPA[d]).filter(d => d !== undefined);
        
        diasNumeros.forEach(d => {
          todosLosDiasNumeros.add(d);
          if (!horariosPorDia[d] || horaInicio < horariosPorDia[d]) {
            horariosPorDia[d] = horaInicio;
          }
        });
        ...
        if (diasNumeros.includes(diaActual)) {
          ...
          if (horaActual < horaInicio) {
            if (horarioAperturaHoy === null || horaInicio < horarioAperturaHoy) {
      ...
      // Intentar parsear formato antiguo: "días · HH:MM–HH:MM"
      match = franja.match(/^([^·]+)·\s*(\d{1,2}):(\d{2})[–\-](\d{1,2}):(\d{2})$/);
    ...
    // Si no abre hoy, buscar el próximo día
    for (let i = 1; i <= 7; i++) {
